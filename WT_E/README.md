...
- **No-activity detection** is gated on whether the participant clicked or moved the cursor at all during the subtask (`markActivity_()`, listening for both `mousemove` and `click` — a touchscreen tap with no dragging never fires `mousemove` alone, which under-detected activity on mobile before this was added; `S.mouseMoved`/`S.firstMoveAt`), **not** on whether any cell was checked — checking nothing can be a genuine, active submission (e.g. a grid the participant correctly judged had no matches left to find). The normal missed/false-positive penalty always applies regardless; if no cursor movement was detected, a **blocking** "Are you still there?" screen then takes precedence over whatever would normally happen next (next subtask, offer, completion) — it does not regenerate a new grid/target, it just holds progress at the current point until acknowledged (`showNoActivityScreen_()`/`acknowledgeNoActivity_()`). Each grid record in `tasks_json` carries a `no_activity` flag and a `first_move_at` timestamp (first cursor movement since that subtask started, or `null`).
...
- **Every hand also gets its own two GENUINELY hand-specific expected-END-position figures** (`Hands` sheet, not `Responses` — see §9), via `eRemFullrandWith_(hand, localPos, POP_MEAN_END)`, which walks that hand's **real** length/live positions/`pInside` from `localPos` forward, conditioning on survival to that point, plus `POP_MEAN_END` for whatever uncertain hands might still follow if this one is survived — both are the expected TOTAL main-sequence length (in cards), not a "further tasks remaining" delta:
  - **`e_end_at_hand_start`** (`localPos=0`) — before any of this hand's own cards are known to have passed: what the sequence's expected total length looks like the moment this hand is dealt.
  - **`e_end_at_pause`** (`localPos=pause`) — conditional on having survived to this hand's own pause: corrected for however many live/orange cards are already gone by within this hand, plus uncertain future hands. `e_end_at_pause = hand.globalStart + pause + eRemFullrandWith_(hand, pause, POP_MEAN_END)`.

  (An earlier version of this exposed only the *first* hand's own `e_remaining`/`e_end_at_pause` directly on the `Responses` row, and additionally split each into a separate "remaining" delta alongside the "end" total — both simplifications since removed: hand-specificity generalised to every hand on `Hands` instead of just the first on `Responses`, and the "remaining" deltas dropped as redundant once the "end" totals were available, since the position offset needed to recover one from the other is already on the row.)

```
altDuration = max(ALT_MIN=2, round( betaNormal(firstHand.e_end_at_pause − firstHand.globalStart − pause, meanHandLength) ))
...
**Mistake penalties are deducted from that section's own bonus, not from task completion pay.** `penalty` is snapshotted per section at the moment it ends and logged (`penalty`, per-section; `total_penalty`, per-respondent). The actual net amount owed is logged explicitly too, not left for downstream analysis to reconstruct — `net_earnings` (`Responses`, per-section) and `final_bonus` (`Meta`, whole-study), both `= earnings − penalty`, clamped at 0. The per-section completion screen shows this net figure too now (`"You earned £X (after £Y in penalties: £Z net)"`) whenever that section actually had a penalty — it used to show only the gross `pay`.
...
bonusReward = max(0, round(trainingBonus + Σ section.earnings − Σ section.penalty, 2))
            = Meta.final_bonus  // same formula, logged to the backend
...
| sections_done, total_earnings, total_grid_pay, total_grids_completed | Running totals. `total_earnings` is GROSS (not yet net of `total_penalty`) — see `final_bonus` below. |
...
| final_bonus | **The actual amount owed at the end of the whole study** — `total_earnings` net of `total_penalty`, clamped at 0 (see §8's "Autokick"). Same figure shown as "Your bonus reward" on the results screen. |
...
| alt_base, alt_width, alt_minus_expected | `alt_base` = the population constant `POP_MEAN_END` (identical to `e_end_at_start`), `alt_width` = the mean hand length (the beta-normal support width), and `alt_duration` minus the FIRST hand's own expected-further-cards figure at its own pause (`Hands.e_end_at_pause` minus that hand's own position — `hand_n=1`) — negative means the alternative was the shorter option in expectation at that offer. |
| e_end_at_start | A **whole-experiment population constant** (`POP_MEAN_END` — same for every section of every respondent, see §6), NOT computed from this section's own hands. The equivalent figure genuinely specific to a real hand's own parameters now lives only on the `Hands` sheet, for every hand (`e_end_at_hand_start`/`e_end_at_pause`) — this row used to repeat just the FIRST hand's own version as `e_remaining`/`e_end_at_pause`, removed as pure duplication of that hand's own row on `Hands`. |
...
| outcome_summary | **This IS the outcome column** — there is no separate plain `outcome` enum any more (removed as pure redundancy, once `outcome_summary` existed to strictly subsume it — see §14). A fixed, human-readable label summarising the section's behaviour (`completed` / `switched` / `forfeited` / `autokicked` / `abandoned_reload`, refined with WHEN/whether a switch or forfeit also happened), purely derived from the other columns here, never a new source of truth — see §14, "Outcome taxonomy," for the full derivation table. |
...
| total_targets, total_found, total_false_pos, total_missed | Aggregated grid accuracy — a genuine sum across every hand plus the alt phase, unlike `tasks_json` below |
| tasks_json | JSON array — the ALT PHASE's own grids only (`no_activity`/`first_move_at` per grid, see §5). The main phase's own grids now live on `Hands.tasks_json` instead, one hand's own cards per row — this column used to mix every hand's cards together with no way to tell which hand a given card was even played in, since which hand a card belongs to is hand-level data, not sequence-level. |
...
| e_end_at_hand_start | **Expected total main-sequence length (cards), as anticipated the moment this hand is dealt** — before any of its own cards are known to have passed. Genuinely accounts for THIS hand's own real `p_inside`/length (unlike `Responses.e_end_at_start`, a flat population constant that never varies by hand), plus `POP_MEAN_END` for whatever uncertain hands might still follow. See §6. |
| e_end_at_pause | Same idea, but **as anticipated at this hand's own `pause`** — conditional on having survived to it, corrected for however many live/orange cards are already gone by within this hand (`eRemFullrandWith_()`, walking this hand's real live positions), plus uncertain future hands. For the FIRST hand of a section, this is the figure `alt_duration`/`alt_minus_expected` (`Responses` sheet) are built around. |
| pause_secs, offer_fired_at | Set at RUNTIME, NOT fixed at generation time like `pause`/`sunk_cost_at_pause`/`left_pattern`/`right_pattern`/the `e_end_at_*` fields above — the moment this hand's own offer screen actually shows (`showOfferScreen()` in `index.html`): `pause_secs` is how long that screen's countdown ran (Uniform[10,45]s, redrawn independently on every offer) — the ENTIRE accept/refuse window for this hand, spanning both the initial ask step and its own follow-up confirm step (the same countdown runs underneath both, uninterrupted — see the next row), `offer_fired_at` is the real wall-clock ISO timestamp of that same moment — `Responses`' `switch_offered_at_task`/`_at_time` only ever capture the FIRST such moment across the whole section, by design, so every later hand's own firing time lives only here. Both blank iff this hand's own offer was wasted (`wasted=1`), since no offer screen was ever shown for it. |
| deliberation_secs | **How long the participant actually took to decide**, in real elapsed seconds — the gap from `offer_fired_at` to whichever moment `decide()` (`index.html`) actually committed a final decision for THIS hand's own offer: accept or refuse, manually confirmed or auto-resolved by the countdown reaching 0 (`stampDeliberation_()`). Deliberately distinct from `pause_secs` (the RANDOMISED length assigned to the countdown, not how long deciding actually took) and from `Responses.switch_decision_secs` (task-level, switch-taken case only, and only the step-1→step-2 confirm-popup gap, not the initial ask step before it) — this is the one true "how long did the pause actually last before a final decision," covering both outcomes. Blank iff this hand's own offer was wasted (`wasted=1`) or fired but was never resolved (a reload discarded the attempt first). |
...
| switched | **Plain 0/1 flag — 1 on the ONE hand whose own offer actually led to a real switch** (`Responses.switch_taken=1`), 0 on every other hand. See `switch_at_task_hand`/`_overall` below for exactly where. |
| forfeited | **Plain 0/1 flag — 1 on the hand the respondent was in when they forfeited**, IF they forfeited during the main phase; 0 on every other hand (including if they forfeited during the alt phase instead, which has no hand to attribute it to). See `forfeit_at_task_hand`/`_overall` below for exactly where. |
| switch_at_task_hand, switch_at_task_overall | Only meaningful when `switched=1` on this row. `_hand` is the card position within that hand; `_overall` is the card position within the whole sequence (same value as `Responses.switch_taken_at_task`). Without these, finding which hand's offer was accepted means cross-referencing that timestamp against every hand's own `globalStart`/`playedLen` range instead. |
| forfeit_at_task_hand, forfeit_at_task_overall | Only meaningful when `forfeited=1` on this row — the card position within that hand, and within the whole sequence (same value as `Responses.forfeit_taken_at_task`). Mirror `switch_at_task_hand`/`_overall` above. |
| tasks_json | **Every grid actually played during THIS hand** — same shape as `Responses.tasks_json` (see its structure below). Card-level accuracy data used to live in one flat, sequence-wide array on `Responses` with no way to tell which hand a given card was played in — genuinely hand-level data, moved here. Empty array if this hand was never reached. |
...
**`tasks_json` array structure** (one element per completed grid — same shape on both `Hands.tasks_json`, this hand's own cards, and `Responses.tasks_json`, the alt phase's):
```json
[
  {
...
- **`alt_duration` is beta-normal, its endpoint centred on the first hand's own `e_end_at_pause` (`Hands` sheet, `hand_n=1`; length centred on that same figure net of position already reached), width = the mean hand length, floored at `ALT_MIN=2`.** Centring the *endpoint* keeps the switch a ~50/50 call in expectation, which centring the length on the population constant alone would not.
...
- **`switch_taken`/`forfeit_taken` are independent, persistent flags.** A participant can switch and *then* forfeit the alternative sequence — a single plain enum value cannot represent that combination on its own, which is exactly why `outcome_summary` exists (below) as the sheet's only outcome column.
...
- **Outcome taxonomy (`outcome_summary`)**: `switch_taken`/`forfeit_taken`/`auto_kicked`/the raw outcome passed to `endTask()` are the real, independent data — accurate but not glanceable as a single label (and there is deliberately no separate plain-enum `outcome` column any more, since `outcome_summary` strictly subsumes it). `computeOutcomeSummary_()` (`index.html`) derives exactly one of the following strings, purely for reading the sheet at a glance:
  ...
  All values (aside from the reload one, set directly by `buildAbandonedRecord_`) are fully derivable from columns already logged (`switch_offered_at_task`, `switch_taken`, `switch_pressed_at_task`, `alt_phase_started_at`, `forfeit_taken`, `auto_kicked`, and the raw outcome `endTask()` was called with) — `outcome_summary` is a read-time convenience, never a new source of truth, and is the sheet's only outcome column (see §9).
